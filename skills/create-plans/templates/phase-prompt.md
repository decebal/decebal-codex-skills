...
<execution_context>
Execute tasks in order. Read listed context files first. Keep changes within
task scope, run each `<verify>` command, and stop at blocking checkpoints. After
all tasks pass, write SUMMARY.md using Outcomes, Files changed, Verification,
...
- Explicit context paths Codex must read
...
<execution_context>
Execute tasks in order. Read listed context files first. Keep changes within
task scope, run each `<verify>` command, and write SUMMARY.md after all gates pass.
</execution_context>
...
This is useless. No XML structure, no context paths, no verification, no specificity.
</bad_examples>
