---
name: create-plans
description: "Create hierarchical project plans optimized for solo Codex development. Use for briefs, roadmaps, phase plans, tasks, verification criteria, and context handoffs. When Chronis or another tracker already owns the queue, use this skill only for per-task execution detail, stored in the task description or .planning/phases/[task-id].md."
---
...
- Context (explicit file paths Codex must read)
...
**Autonomous execution:** Plans without checkpoints may execute in a focused
subagent when environment supports it and file ownership is disjoint.
...
- Write `.env` files with available file-editing tools
...
<domain_expertise>
**Domain expertise comes from available Codex skills.**

Before creating roadmap or phase plans, determine if domain expertise should be loaded.

<scan_domains>
Inspect current skill catalog for relevant domain skills. Do not assume a fixed
filesystem location: plugin, repository, and user installations resolve from
different roots.

**If no domain skill is available:** Proceed without domain expertise.
</scan_domains>
...
|----------|--------------|
| "macOS", "Mac app", "menu bar", "AppKit", "SwiftUI desktop" | macos-apps |
| "iPhone", "iOS", "iPad", "mobile app", "SwiftUI mobile" | iphone-apps |
| "Unity", "game", "C#", "3D game", "2D game" | unity-games |
| "MIDI", "MIDI tool", "sequencer", "MIDI controller", "music app", "MIDI 2.0", "MPE", "SysEx" | midi |
| "Agent SDK", "Codex SDK", "agentic app" | with-agent-sdk |
| "Python automation", "workflow", "API integration", "webhooks", "Celery", "Airflow", "Prefect" | python-workflow-automation |
| "UI", "design", "frontend", "interface", "responsive", "visual design", "landing page", "website design", "Tailwind", "CSS", "web design" | frontend-design |

If domain inferred, confirm:
```
Detected: [domain] project → [installed skill name, for example `$frontend-design`]
...
[... any other relevant installed skills]
...
**Step 1: Resolve and read domain `SKILL.md` from current Codex skill catalog.**

Do not hard-code `~/.agents/skills`; plugin and repository skills may live elsewhere.
...
Resolve references relative to selected skill directory. Load only references
needed for current phase; never bulk-load an entire reference tree.
...
**If SKILL.md does not have a reference index:** inspect reference filenames and
load only likely matches. Do not fall back to loading every file.
</load_domain>
...
| "execute", "run", "do it", "build it", 2 (has structure) | **EXIT SKILL** → Run the selected plan with `codex exec - < path/to/PLAN.md` |
...
**Critical:** Plan execution should not invoke this planning skill again. Feed the
reviewed PLAN.md directly to `codex exec -` so execution receives only plan and
...
<prime_integration>
**Whenever a PLAN.md, RESEARCH.md, or BRIEF.md is written** (any of the workflows: create-brief, create-roadmap, plan-phase, plan-chunk, research-phase), also register it in the per-repo `.prime/` graph when that MCP server is configured, so future planning calls can recall related prior plans and execution can attach outcomes.

For each artifact written:

1. `mcp__prime__prime_add_node`:
   - type: one of `"brief"`, `"roadmap"`, `"plan"`, `"research"`
   - properties: `{ name: <slug, e.g. "01-03-database-setup">, file: <relative path, e.g. ".planning/phases/01-foundation/01-03-PLAN.md" or "docs/plans/2026-05-20-prime-skill-upgrade.md">, phase: <"01"|"02"|... if applicable>, domain: <inferred from request or loaded expertise>, created_at: <today YYYY-MM-DD> }`
   - Capture entity_id.

2. `mcp__prime__prime_embed { id: <entity_id>, text: <artifact objective + 1-line intent, ~1–3 sentences> }` — server embeds via in-process fastembed. Enables semantic recall ("similar past plans in this codebase?") on future planning invocations.

3. For each path listed in the artifact's context section, ensure a `file` node exists (search by `path` property, create if missing) and `mcp__prime__prime_add_edge { source: <plan>, target: <file>, relation: "references" }`.

4. For phase ordering, add `mcp__prime__prime_add_edge { source: <later plan>, target: <earlier plan>, relation: "depends_on" }` based on phase number sequence.

5. For SUMMARY.md (phase outcome): `mcp__prime__prime_add_node { type: "plan_outcome", properties: { plan_file, status, completed_at } }` + edge `outcomes` to the plan node. Outcomes are queried via graph traversal, no embed needed.

6. Before creating a new BRIEF/ROADMAP/PLAN, optionally `mcp__prime__prime_recall { text: <new plan's objective>, node_type: "plan", top_k: 5 }` to surface similar prior plans for reference.

Best-effort: if any `mcp__prime__*` call errors, log and continue. Files are canonical, Prime is the index. Requires `allsource-prime ≥ 0.21.3` for text-on-embed.
</prime_integration>
...
- Plan execution feeds the reviewed PLAN.md to `codex exec -`
- Each written artifact (BRIEF, ROADMAP, PLAN, RESEARCH, SUMMARY) registered as a node in `.prime/` (best-effort; silently skipped if Prime MCP unreachable)
</success_criteria>
