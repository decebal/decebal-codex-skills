...
- Main context stays focused on checkpoints and integration
- Independent segments can run concurrently when file ownership is disjoint
- Smaller assignments are easier to verify and retry
...
```
Delegate to one Codex subagent when subagents are available. Give it explicit
ownership of plan files and remind it not to revert concurrent edits:
...
2. For each segment in order:
   ...
   B. If routing = Subagent:
      ```
      Delegate to a Codex subagent with explicit file ownership:
...
1. Read every context file listed in the prompt
